"""Native cognis-connect emit for chainreserve — forward findings to any platform.

Maps chainreserve's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    chainreserve ... --format json | chainreserve-emit --to stix
    chainreserve-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('entity', '') + ' ' + rec.get('asset', '')
